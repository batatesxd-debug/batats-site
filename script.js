...
function showSlide(index){
  ...
  gsap.fromTo(activeSlide.querySelector(".slide-box"), {y:50,opacity:0,scale:0.9},{y:0,opacity:1,scale:1,duration:1.2,ease:"power4.out"});
  const bullets=activeSlide.querySelectorAll("ul li");
  bullets.forEach((li,i)=>{gsap.to(li,{opacity:1,x:0,duration:0.8,delay:0.6+i*0.4,ease:"power3.out"});});
}
showSlide(0);
document.addEventListener("click",()=>{current=(current+1)%slides.length;showSlide(current);});
